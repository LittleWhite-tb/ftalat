...
for i in range(size):
   ...
   scriptFile.write("\"" + sys.argv[2] + "\" using (2.7/(16/$1)):(${0} != 0 ? ((${0})) : 1/0) axes x1y1 title \"{1} Ghz\" with linespoints palette frac {3}".format(i+2,(2.7/(16/sortedKeys[i]))/1000000.0,float(sortedKeys[size-1]),(1.0/(len(sortedKeys)))*i))
   if i != size-1:
      scriptFile.write(", \\\n")
